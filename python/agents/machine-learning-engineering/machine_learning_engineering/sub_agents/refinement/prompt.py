...
- The solution uses a **skrub DataOps pipeline**. Your ablation study should test skrub pipeline components.
...
- Consider ablating skrub DataOps components inside `.skb.apply(...)`: e.g., swap TableVectorizer encoders (GapEncoder vs MinHashEncoder vs OneHotEncoder), remove DatetimeEncoder, change cardinality thresholds, or simplify the estimator.
...
ABLATION_INSTR_DEFAULT = """# Introduction
- You are a Kaggle grandmaster attending a competition.
- In order to win this competition, you need to perform an ablation study on the current Python solution to know which parts of the code contribute the most to the overall performance.
- We will now provide a current Python solution.
...
- You are a Kaggle grandmaster attending a competition.
- In order to win this competition, you need to perform an ablation study on the current Python solution to know which parts of the code contribute the most to the overall performance.
- The solution uses a **skrub DataOps pipeline**.
- We will now provide a current Python solution.
...
- Consider ablating skrub DataOps components: encoder choices, TableVectorizer settings, `.skb.apply(...)` steps.
- For each ablation, print out how the modification affects the model's performance.
...
ABLATION_SEQ_INSTR_DEFAULT = """# Introduction
- You are a Kaggle grandmaster attending a competition.
- In order to win this competition, you need to perform an ablation study on the current Python solution to know which parts of the code contribute the most to the overall performance.
- We will now provide a current Python solution.
- We will also provide the summaries of previous ablation studies.
...
- The solution uses a **skrub DataOps pipeline**. Your improvement plan must maintain the skrub DataOps graph structure (`skrub.var` -> `.skb.apply(...)` -> `.skb.make_learner`). Do not convert it to a scikit-learn `Pipeline`/`make_pipeline`.
...
- Improvements should work within the skrub DataOps graph — e.g., tuning encoder choices (GapEncoder, MinHashEncoder, DatetimeEncoder) inside `.skb.apply(...)`, adjusting TableVectorizer settings, or improving the estimator applied via `.skb.apply(model, y=y)`.
...
EXTRACT_BLOCK_AND_PLAN_INSTR_DEFAULT = """# Introduction
- You are a Kaggle grandmaster attending a competition.
- In order to win this competition, you need to extract a code block from the current Python solution and improve the extracted block for better performance.
- Your suggestion should be based on the ablation study results of the current Python solution.
- We will now provide the current Python solution and the ablation study results.
...
EXTRACT_BLOCK_AND_PLAN_SEQ_INSTR_DEFAULT = """# Introduction
- You are a Kaggle grandmaster attending a competition.
- In order to win this competition, you need to extract a code block from the current Python solution and improve the extracted block for better performance.
- Your suggestion should be based on the ablation study results of the current Python solution.
- We will now provide the current Python solution and the ablation study results.
...
- The solution uses a **skrub DataOps pipeline**. Your improvements must maintain the skrub DataOps graph structure (`skrub.var` -> `.skb.apply(...)` -> `.skb.make_learner`). Do not convert it to a scikit-learn `Pipeline`/`make_pipeline`.
...
# Your task
- Suggest a better plan to improve the above code block while maintaining the skrub DataOps graph structure (`skrub.var` -> `.skb.apply(...)` -> `.skb.make_learner`); do not convert it to a scikit-learn `Pipeline`/`make_pipeline`.
- The suggested plan must be novel and effective.
- Consider skrub-specific improvements: different encoder choices (GapEncoder, MinHashEncoder, DatetimeEncoder) inside `.skb.apply(...)`, TableVectorizer configuration, or better estimators applied via `.skb.apply(model, y=y)`.
...
PLAN_REFINEMENT_INSTR_DEFAULT = """# Introduction
- You are a Kaggle grandmaster attending a competition.
- In order to win this competition, you have to improve the code block for better performance.
- We will provide the code block you are improving and the improvement plans you have tried.
...
- The solution uses a **skrub DataOps pipeline**. Maintain the skrub pipeline structure.
...
- Maintain the skrub DataOps graph structure (`skrub.var` -> `.skb.apply(...)` -> `.skb.make_learner`); do not convert it to a scikit-learn `Pipeline`/`make_pipeline`.
...
IMPLEMENT_PLAN_INSTR_DEFAULT = """# Introduction
- You are a Kaggle grandmaster attending a competition.
- In order to win this competition, you need refine the code block for better performance based on the improvement plan.
- We will now provide the code block and the improvement plan.
