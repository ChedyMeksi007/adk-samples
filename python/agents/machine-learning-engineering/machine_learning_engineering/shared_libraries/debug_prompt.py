...
- Maintain the skrub DataOps graph structure (`skrub.var` -> `.skb.mark_as_X`/`mark_as_y` -> `.skb.apply(...)` -> `.skb.make_learner`). Fix the bug within the DataOps graph; do not convert it to a scikit-learn `Pipeline`/`make_pipeline`.
...
BUG_REFINE_INSTR_DEFAULT = """# Task description
{task_description}
